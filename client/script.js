...
// const farm = document.querySelector('form')
// PROBLEM 1
/*
...
function colorrevert(event){
    event.target.style.backgroundColor = ''
    event.target.style.color = ''
}
sayhelloBtn.addEventListener('mouseover', colorChange)
sayhelloBtn.addEventListener("mouseout",colorrevert)
...
axios.get(`http://localhost:3000/animals`).then(res => {
  for(let i=0; i<res.data.length; i++){
      ...
      creature.textContent = res.data[i] 
      document.querySelector('body').appendChild(creature)
  }
})
}
...
*/
let grabCmd= document.querySelector('#query-button')
function grabber(event){
    axios.get(`${baseURL}query-test/?animal=monkey`).then(res => {
        console.log(res.data)
    })
        
}
grabCmd.addEventListener('click', grabber)
...
/*
    ...
    field and a button. The input field should have a placeholder that tells the user to 
    enter a food. And the button should indicate that it will add food into a list. 
...
const createFood = () =>{
    ...
    axios.post(`${baseURL}food`,body)
        .then(res => {
            ...
            const fud = document.createElement('p')
             fud.textContent = res.data.pop()
            document.querySelector('body').appendChild(fud)
    })
}
const billyJoe = document.querySelector('form')
billyJoe.addEventListener('submit',createFood)